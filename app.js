...
function createSingleMediaElement(item, fit = "contain") {
  ...
  if (item.type.startsWith("video/")) {
    // For thumbnails (non-controls context) show a styled placeholder
    // For full preview (controls visible) show actual video player
    if (fit === "cover") {
      const wrap = document.createElement("div");
      wrap.style.cssText = "position:relative;width:100%;height:100%;background:#0f172a;display:flex;align-items:center;justify-content:center;";
      wrap.innerHTML = `
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="12" cy="12" r="12" fill="rgba(255,255,255,0.15)"/>
          <polygon points="10,8 10,16 17,12" fill="white"/>
        </svg>
        <div style="position:absolute;bottom:8px;left:10px;color:rgba(255,255,255,0.6);font-size:11px;font-family:Inter,sans-serif;">${item.name || "Video"}</div>`;
      return wrap;
    }
    // Show a click-to-play placeholder until user interacts
    const wrap = document.createElement("div");
    wrap.style.cssText = "position:relative;width:100%;height:100%;min-height:200px;background:#0f172a;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:8px;";
    wrap.innerHTML = `
      <svg width="56" height="56" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="12" cy="12" r="12" fill="rgba(255,255,255,0.15)"/>
        <polygon points="10,8 10,16 17,12" fill="white"/>
      </svg>
      <div style="position:absolute;bottom:12px;left:14px;color:rgba(255,255,255,0.6);font-size:12px;font-family:Inter,sans-serif;max-width:80%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item.name || "Video"}</div>
      <div style="position:absolute;top:10px;right:12px;background:rgba(255,255,255,0.15);color:white;font-size:10px;font-family:Inter,sans-serif;padding:3px 8px;border-radius:20px;">VIDEO</div>`;
    wrap.addEventListener("click", () => {
      ...
      video.autoplay = true;
      video.preload = "auto";
      video.style.cssText = "width:100%;height:100%;object-fit:" + fit + ";border-radius:8px;";
...
function createMediaCard(item) {
  ...
  // Inject inline approver checklist into comments side panel
  ...
  const commentsSide = fragment.querySelector(".comments-side");
  ...
  // Remove media-top-actions entirely if empty (no approver selector inside)
  // Approvers are now shown as inline checklist in the comments side panel

  // Build one approve button per approver in the approve-col
  ...
  if (item.approvers && item.approvers.length > 0) {
    item.approvers.forEach(approverName => {
      ...
      btn.addEventListener("click", () => toggleApproval(item.id));
      approveCol.appendChild(btn);
    });
  } else {
    const placeholder = document.createElement("div");
    placeholder.className = "approve-placeholder";
    placeholder.textContent = "Select approvers";
