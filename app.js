...
function createMediaCard(item) {
  ...
  if (item.approvers && item.approvers.length > 0) {
    item.approvers.forEach(approverName => {
      ...
      btn.type = "button";
      btn.className = "approve-icon-btn" + (item.status === "Approved" ? " approved" : "");
      btn.title = approverName;
      btn.innerHTML = `
        <span class="tick-icon">${item.status === "Approved" ? "✔" : "✔"}</span>
    ...
    // No approver selected yet — show placeholder
    const placeholder = document.createElement("div");
    placeholder.className = "approve-placeholder";
    placeholder.textContent = "No approver";
    approveCol.appendChild(placeholder);
  }

  // Hide the old approve button from template (now handled above)
